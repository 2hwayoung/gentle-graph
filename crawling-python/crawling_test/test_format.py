...
class App:
    ...
    def create_content_and_match_with_info(self, url, title, date, n_comment, n_reactions, recommend, like, impress, angry, sad, crawl_time, category, rank, creator, platform):
        ...
        with self.driver.session() as session:
            ...
            if len(result) == 0:
                result = session.write_transaction(
                    self._create_content_and_match_with_info, url, title, date, n_comment, n_reactions, recommend, like, impress, angry, sad, crawl_time, category, rank, creator, platform
                )
            else:
                 # 해당 category에 content가 존재하는지 확인
                result = session.read_transaction(self._find_and_return_category, url, crawl_time, category)
                if len(result) == 0:
                    result = session.write_transaction(
                        self._match_content_with_info, url, crawl_time, category, rank, platform
                    )

    def create_content_and_match_with_info_classified_by_age(self, url, title, date, n_comment, n_reactions, recommend, like, impress, angry, sad, crawl_time, category, age, sex, rank, creator, platform):
        '''[category: age] create content and match with creator, type, rank'''
        with self.driver.session() as session:
            result = session.read_transaction(self._find_and_return_content, url, crawl_time)
            if len(result) == 0:
                result = session.write_transaction(
                    self._create_content_and_match_with_info_age, url, title, date, n_comment, n_reactions, recommend, like, impress, angry, sad, crawl_time, category, age, sex, rank, creator, platform
                )
            else:
                # 해당 category에 content가 존재하는지 확인
                result = session.read_transaction(self._find_and_return_category_age, url, crawl_time, category, age, sex)
                if len(result) == 0:
                    result = session.write_transaction(
    ...
    @staticmethod
    def _create_content_and_match_with_info(tx, url, title, date, n_comment, n_reactions, recommend, like, impress, angry, sad, crawl_time, category, rank, creator, platform):
        '''create_content_and_match_with_info method'''
        query = (
            "CREATE (c:Content:" + crawl_time + " { url: $url, title: $title, date: $date, n_comment: $n_comment, n_reactions: $n_reactions, recommend: $recommend, like: $like, impress: $impress, angry: $angry, sad: $sad }) "
            "WITH c "
        ...
        result = tx.run(query, url=url, title=title, date=date, n_comment=n_comment,\
            n_reactions=n_reactions, recommend=recommend, like=like, impress=impress, angry=angry, sad=sad, \
                category=category, rank=rank, creator=creator, platform=platform)

    @staticmethod
    def _find_and_return_category(tx, url, crawl_time, category):
        '''create_content_and_match_with_info method'''
        query = (
            "MATCH (c: Content:"+crawl_time+" { url: $url }) "
            "WITH c "
            "MATCH (c)<-[:INCLUDES]-(t:Type { name: $category }) "
            "RETURN t.name AS name"
        )
        result = tx.run(query, url=url, category=category)
        res = [row["name"] for row in result]
        if res == None:
            return None
        else:
    ...
    @staticmethod
    def _create_content_and_match_with_info_age(tx, url, title, date, n_comment, n_reactions, recommend, like, impress, angry, sad, crawl_time, category, age, sex, rank, creator, platform):
        '''create_content_and_match_with_info_classified_by_age method'''
        query = (
            "CREATE (c:Content:" + crawl_time + " { url: $url, title: $title, date: $date, n_comment: $n_comment, n_reactions: $n_reactions, recommend: $recommend, like: $like, impress: $impress, angry: $angry, sad: $sad }) "
            "WITH c "
            "MATCH (t:Type { name: $category, age: $age, sex: $sex  }) "
            "MATCH (r:Rank { n: $rank, platform: $platform }) "
            "MATCH (cr:Creator) WHERE cr.name = $creator AND cr.type = 'press' "
            "MERGE (c)<-[:INCLUDES]-(t) "
            "MERGE (c)<-[:RANKS]-(r) "
        ...
        result = tx.run(query, url=url, title=title, date=date, n_comment=n_comment,\
            n_reactions=n_reactions, recommend=recommend, like=like, impress=impress, angry=angry, sad=sad, \
                category=category,  age=age, sex=sex, rank=rank, creator=creator, platform=platform)

    @staticmethod
    def _find_and_return_category_age(tx, url, crawl_time, category, age, sex):
        '''create_content_and_match_with_info_classified_by_age method'''
        query = (
            "MATCH (c: Content:"+crawl_time+" { url: $url }) "
            "WITH c "
            "MATCH (c)<-[:INCLUDES]-(t:Type { name: $category, age: $age, sex: $sex }) "
            "RETURN t.name AS name"
        )
        result = tx.run(query, url=url, category=category, age=age, sex=sex)
        res = [row["name"] for row in result]
        if res == None:
            return None
        else:
    ...
    def _match_content_with_info_age(tx, url, crawl_time, category, age, sex, rank, platform):
        ...
        result = tx.run(query, url=url, category=category, age=age, sex=sex, rank=rank, platform=platform)
...
if __name__ == "__main__":
    ...
    # bolt_url = "bolt://165.132.172.73:7687"
    user = "neo4j"
    password = "neo4j123"
    ...
    file_list_py = [file for file in file_list if file.startswith('youtube_data') and '25' not in file and '26' not in file]
    for i in file_list_py:
        print(path+i)
        with open((path+i), 'r') as f:
            json_data = json.load(f)
        for j in range(len(json_data)):
        #     for item in json_data[j]:
        #         try:
        #             value = int(json_data[j][item])
        #             if value == int(json_data[j][item]):
        #                 json_data[j][item] = value
        #         except:
        #             pass

            # json_data[j]['n_reaction_good'] = str_to_int(str(json_data[j]['n_reaction_good']))
            # json_data[j]['n_reaction_bad'] = str_to_int(str(json_data[j]['n_reaction_bad']))
            # json_data[j]['n_reactions'] = int(json_data[j]['n_reaction_recommend']) + int(json_data[j]['n_reaction_like']) + \
            #      int(json_data[j]['n_reaction_impress']) + int(json_data[j]['n_reaction_angry']) + int(json_data[j]['n_reaction_sad'])
        #     json_data[j]['n_reactions'] = json_data[j]['n_reaction_good'] + json_data[j]['n_reaction_bad']
        # out_dir = '/home/capje/crawling/data/'+i
        # print(out_dir)
