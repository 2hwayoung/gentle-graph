...
def crawl_content(soup, driver, category, ranking_box_class, num, crawl_time):
...
# 연령별 뉴스
def crawl_content_by_age(soup, driver, category, crawl_time):
...
def crawling(chrome_driver_path: str):
    ...
    # for i in tqdm(range(len(tags))):
       # tag = tags[i]
        ...
        if main_tag == "popular":
            news_list.extend(crawl_content(soup, driver, tag, 'rank_num rank_popular', 50, crawl_time))
        elif main_tag == "kkomkkom":
            news_list.extend(crawl_content(soup, driver, tag, 'rank_num rank_popular', 30, crawl_time))
        elif main_tag == "bestreply":
            news_list.extend(crawl_content(soup, driver, main_tag, 'rank_num', 50, crawl_time))
        elif main_tag == "age":
            news_list.extend(crawl_content_by_age(soup, driver, main_tag, crawl_time))
    return news_list
