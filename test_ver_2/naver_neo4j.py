...
class App:
    ...
    def match_content_and_keyword(self, keyword, url, title, date, n_view, n_comment, good, warm, sad, angry, want ):
        with self.driver.session() as session:
            result = session.read_transaction(
                self._find_content_and_keyword, keyword, url, title, date, n_view, n_comment, good, warm, sad, angry, want
            )
            if len(result) == 0:
                session.write_transaction(
                    self._match_content_and_keyword, keyword, url, title, date, n_view, n_comment, good, warm, sad, angry, want
                )

    # 모든 항목 content의 property로 넣기
    def create_content(self, url, title, date, n_view, n_comment, good, warm, sad, angry, want, crawl_time, category, rank, creator, platform):
        with self.driver.session() as session:
            session.write_transaction(
                self._create_content, url, title, date, n_view, n_comment, good, warm, sad, angry, want, crawl_time, category, rank, creator, platform
            )
    ...
    @staticmethod
    def _find_content_and_keyword(tx, keyword, url, title, date, n_view, n_comment, good, warm, sad, angry, want ):
        query = (
            "MATCH (c: Content { url: $url, title: $title, date: $date, n_view: $n_view, n_comment: $n_comment, good: $good, warm: $warm, sad: $sad, angry: $angry, want: $want })-[:EXTRACTS]->(k: Keyword { name: $keyword}) "
            "RETURN k.name AS name"
            )
    ...
    @staticmethod
    def _match_content_and_keyword(tx, keyword, url, title, date, n_view, n_comment, good, warm, sad, angry, want ):
        query = (
            "MATCH (c: Content { url: $url, title: $title, date: $date, n_view: $n_view, n_comment: $n_comment, good: $good, warm: $warm, sad: $sad, angry: $angry, want: $want }) "
    ...
    @staticmethod
    def _create_content(tx, url, title, date, n_view, n_comment, good, warm, sad, angry, want, crawl_time, category, rank, creator, platform):
        query = (
